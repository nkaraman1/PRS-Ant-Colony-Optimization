...
__forceinline__ __device__ int NextCity(struct ant *ants_d, int pos, float *distances_d, double *hormone_d, curandState *state_d ){
	...
	for (to = 0; to < CITIES; to += 5) {
		if(ants_d[pos].visited[to] == 0) denom += antProduct(from, to, hormone_d, distances_d);
		if(ants_d[pos].visited[to + 1] == 0) denom += antProduct(from, to + 1, hormone_d, distances_d);
		if(ants_d[pos].visited[to + 2] == 0) denom += antProduct(from, to + 2, hormone_d, distances_d);
		if(ants_d[pos].visited[to + 3] == 0) denom += antProduct(from, to + 3, hormone_d, distances_d);
